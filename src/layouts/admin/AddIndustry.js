...
function AddIndustry() {
  ...
  const handleSubmit = (e) => {
    ...
    if (
      !formInput.industry_name === "" ||
      /^\s*$/.test(formInput.industry_name) ||
      formInput.industry_name.length === 0
    ) {
      fieldErrors.industry_name = "Industry Name is required.";
    } else if (!/^[a-zA-Z\s]*$/.test(formInput.industry_name)) {
      fieldErrors.industry_name =
        "Industry Name should only contain alphabets and spaces.";
    }
    ...
    if (Object.keys(fieldErrors).length === 0) {
      ...
      formData.append("industry_name", formInput.industry_name);

      axios
        .post(`/api/industry`, formData, {
